...
/* Mesma cascata do engine: os padroes, os segredos da maquina, e por ultimo o
   arquivo do show (ENV_FILE=.env.yuna). Sem o do meio, este script perdia as
   chaves do Browserbase ao rodar com o env da Yuna. */
...
/* Qualquer agente do elenco, nao so os dois do Conatus. Era uma lista fixa e
   deixava a Yuna sem como logar na pump — o contexto do Browserbase e o que
   carrega o login entre sessoes, entao sem isto ela nunca entraria. */
const AGENTE = (process.argv[2] || "sable").toLowerCase();
if (!/^[a-z][a-z0-9_-]{1,20}$/.test(AGENTE)) {
  console.error(`agente invalido: ${AGENTE}`);
  process.exit(1);
}
