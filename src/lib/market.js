...
/* PUMP_API_BASE existe para o TESTE. Modulo ES e read-only: um probe nao
   consegue trocar `pumpCoin` por uma versao de mentira, e sem um jeito de
   apontar pra outro lugar so restaria bater na pump.fun de verdade — o que faz
   um teste depender da internet e do humor de uma API de terceiro. */
const PUMP = process.env.PUMP_API_BASE || "https://frontend-api-v3.pump.fun";
...
export async function pumpCoin(mint) {
  ...
  return {
    ...
    /* A pump parou de preencher real_sol_reserves (conferido em 31/08/2026:
       vem 0 ate numa moeda de $9M negociando agora). Quem sabe dizer se da pra
       revender e o par mcap + ultima negociacao. */
